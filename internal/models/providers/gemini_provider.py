...
"""
Gemini-compatible provider — SOVEREIGN LOCAL MODE.

Routes all requests through the local TSM/Ollama inference endpoint.
No Google API key or google-generativeai SDK is required.
"""
...
class GeminiProvider(BaseModelProvider):
    """
    Local inference provider masquerading as Gemini.
    All calls are forwarded to the Ollama-compatible local server.
    """

    def __init__(self, api_key: Optional[str] = None, **kwargs):
        super().__init__(api_key, **kwargs)
        # api_key intentionally ignored
    ...
    async def generate(
        ...
        **kwargs,
        ...
        """Generate using local inference (previously Google Gemini)."""
