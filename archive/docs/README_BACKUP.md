...
### 🧠 Smart Routing
- Complex reasoning → Cloud LLM (via API)
- Code analysis → Cloud LLM (via API)
- Simple queries → Cloud LLM (via API)
- **Sensitive data → Local model (HuggingFace)**

All automatic based on content. Works with any cloud provider (OpenAI, Anthropic, Google, etc.).
