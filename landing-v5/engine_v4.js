...
/* shared scroll state — drives the core morph and camera pull */
const scr = { target: 0, smooth: 0 };
function readScroll() {
  const max = Math.max(1, document.body.scrollHeight - innerHeight);
  scr.target = Math.min(1, Math.max(0, scrollY / max));
}
...
async function initWebGPU() {
  ...
  /* ---------- CORE — persistent procedural mesh anchoring the eye ---------- */
  // A slow wireframe torus knot in signal colour. Bloom + additive blending
  // gives it the "hot lamp" halo the brand wants. Scroll-tied scale/rotation
  // are applied in the frame loop so the core grows + accelerates as you read.
  const coreGeom = new THREE.TorusKnotGeometry(5.4, 1.35, 220, 32);
  const coreMat  = new THREE.MeshBasicNodeMaterial({
    color: 0xC7F23E, wireframe: true, transparent: true,
    opacity: 0.55, blending: THREE.AdditiveBlending, depthWrite: false,
  });
  const core = new THREE.Mesh(coreGeom, coreMat);
  scene.add(core);

  /* ---------- ALT CORES — geometric mood changes per scroll band ----------
     Three eye-anchors crossfade as the page is read: the wireframe TorusKnot
     belongs to the hero band, an Icosahedron takes the middle, an Octahedron
     finishes. Same signal colour + additive blending so bloom carries through
     transitions; opacity weights are driven from the frame loop below. */
  const altMat1 = new THREE.MeshBasicNodeMaterial({
    color: 0xC7F23E, wireframe: true, transparent: true,
    opacity: 0, blending: THREE.AdditiveBlending, depthWrite: false,
  });
  const altMesh1 = new THREE.Mesh(new THREE.IcosahedronGeometry(6.6, 1), altMat1);
  scene.add(altMesh1);

  const altMat2 = new THREE.MeshBasicNodeMaterial({
    color: 0xC7F23E, wireframe: true, transparent: true,
    opacity: 0, blending: THREE.AdditiveBlending, depthWrite: false,
  });
  const altMesh2 = new THREE.Mesh(new THREE.OctahedronGeometry(7.0, 2), altMat2);
  scene.add(altMesh2);
  ...
  try {
    ...
    // strength 1.8 (was 0.9), radius 0.85, threshold 0.6 — only hot pixels bloom,
    // so the lamp + core wireframe halo hard while the muted backdrop stays clean.
    const bloomPass = bloom(color, 1.8, 0.85, 0.6);
    ...
    const compose = Fn(() => {
      const suv = screenUV;
      // Directional aberration = base radial swell + cursor-velocity directional.
      // ~2× v4 amplitude so the fringe is visible at idle and dramatic on motion.
      const radial = suv.sub(vec2(0.5, 0.5));
      const amt = float(0.003).add(uAberr.mul(0.02));
      const dirOff = vec2(uDir.x, uDir.y).mul(amt.mul(1.4));
      const oR = radial.mul(amt.mul(2.0)).add(dirOff.mul(1.8));
      const oB = radial.mul(amt.mul(2.0)).add(dirOff.mul(2.2)).negate();
      const r = color.sample(suv.add(oR)).r;
      const g = color.sample(suv).g;
      const b = color.sample(suv.add(oB)).b;
      let col = vec3(r, g, b);
      // film grain — louder than v4 (0.09 vs 0.05) for filmic mass
      const n = suv.add(time.mul(0.5)).mul(vec2(1213.0, 871.0));
      const grain = n.x.add(n.y).sin().mul(43758.5453).fract();
      col = col.add(vec3(grain.sub(0.5)).mul(0.09));
      // vignette — darken edges to focus the eye on the core
      const vign = float(1.0).sub(radial.length().mul(0.9));
      col = col.mul(vign);
      // very subtle scanlines (terminal/CRT feel; ~3% modulation)
      const scan = float(0.97).add(suv.y.mul(800.0).sin().mul(0.03));
      col = col.mul(scan);
      return vec4(col, 1.0);
    });
  ...
  function frame() {
    ...
    // smoothed scroll progress drives camera dolly + core morph
    scr.smooth += (scr.target - scr.smooth) * 0.08;

    // camera parallax + drift; pulls ~5 units closer at full scroll
    ...
    cam.position.z = 22 + Math.sin(t*0.06)*0.6 - scr.smooth * 5;
    cam.lookAt(0,0,0);
    ...
    // CORE — slow rotation that accelerates on scroll; grows and brightens
    core.rotation.x += 0.002 + scr.smooth * 0.004;
    core.rotation.y += 0.003 + scr.smooth * 0.006;
    const s = 1.0 + scr.smooth * 0.4;
    core.scale.set(s, s, s);

    // PER-SECTION MORPH — three eye-anchors crossfade by scroll band.
    // Triangle-window weights with gapless overlap: at every scroll point at
    // least one mesh is at full brightness OR two are blending at 50% each, so
    // the page never feels empty at a transition boundary.
    const p = scr.smooth;
    const w0 = Math.max(0, 1 - p * 2);                   // hero band   (1 at 0, 0 at 0.5)
    const w1 = Math.max(0, 1 - Math.abs(p - 0.5) * 2);   // middle band (peak at 0.5)
    const w2 = Math.max(0, p * 2 - 1);                   // outro band  (0 at 0.5, 1 at 1)
    core.material.opacity     = (0.55 + p * 0.25) * w0;
    altMesh1.rotation.x      += 0.0015 + p * 0.003;
    altMesh1.rotation.y      -= 0.0025 + p * 0.004;
    altMesh1.scale.setScalar(0.9 + p * 0.3);
    altMesh1.material.opacity = 0.6 * w1;
    altMesh2.rotation.x      -= 0.001 + p * 0.003;
    altMesh2.rotation.z      += 0.002 + p * 0.005;
    altMesh2.scale.setScalar(0.85 + p * 0.5);
    altMesh2.material.opacity = 0.7 * w2;
